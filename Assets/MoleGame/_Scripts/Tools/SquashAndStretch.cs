...
/// 
/// Parent (where the logic (and optionally rigidbody lies)
/// - THIS
/// - - Model / sprite
/// 
...
/// </summary>
[AddComponentMenu("Tools/Movement/SquashAndStretch")]
public class SquashAndStretch : MonoBehaviour
{
    ...
    [Tooltip("This component will apply squash and stretch based on velocity (either position based or computed from a Rigidbody. It has to be put on an intermediary level in the hierarchy, between the logic (top level) and the model (bottom level).")]
    ...
    [Header("Debug")]
    public Vector3 Velocity;  // The current velocity of the parent object
    ...
    /// </summary>
    protected virtual void ApplySquashAndStretch()
    {
    ...
    protected virtual void ComputeVelocityAndDirection()
    {
        ...
        RemappedVelocity = MathTools.Remap(VelocityMagnitude, 0f, MaximumVelocity, 0f, 1f);
        ...
        if (AutoSquashOnStop)
        {
            ...
            else if (_movementStarted)
            {
                ...
                _squashing = true;
                float duration = MathTools.Remap(_lastVelocity, 0f, MaximumVelocity, SquashDuration.x, SquashDuration.y);
                float intensity = MathTools.Remap(_lastVelocity, 0f, MaximumVelocity, SquashIntensity.x, SquashIntensity.y);
    ...
    protected virtual void ComputeNewLocalScale()
    {
        if (_squashing)
        {
            float elapsed = MathTools.Remap(TimescaleTime - _squashStartedAt, 0f, _squashDuration, 0f, 1f);
